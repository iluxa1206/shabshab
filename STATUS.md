...
**Миграция данных (три шага, порядок обязателен).**
1. `scripts/recalc_honest_history.py --days 400` — 601 флоатер, ~1,5 ч, лог
   `data/logs/recalc_honest.log`.
2. `scripts/backfill_hourly_bars.py --days 365 --no-ticks` — только ПОСЛЕ шага 1:
   вдвоём они дерутся за MOEX и за `portfolio.db`, ловя throttle и взаимные блокировки.
   Автозапуск по завершении шага 1 — `/root/floaters/chain_bars.sh` (ждёт ХОСТОВЫМ
   pgrep: в образе его нет, и первая версия скрипта из-за этого стартовала немедленно).
3. Ночной прогрев 03:00 МСК (`nightly_spread_warm`, топ-200 × 150 дн) подбирает остальное сам.

...
- **Сделки поштучно + курсив подписи** (25.08): одно сообщение = одна сделка
  (`tg_notify._group` бьёт по `trade_id`, поле добавлено в `block_trades._match`)
  — пачкой три принта по одной бумаге отличались только объёмом. Имя алерта в

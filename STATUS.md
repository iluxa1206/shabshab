...
- Амортизируемые: z-модель (zspread.py) УЧИТЫВАЕТ амортизацию (2026-07-02: принципал по датам bondization amortizations, купоны от остаточного номинала = сумма будущих амортизаций; поле face купонов MOEX ненадёжно — не проецируется/стейл). DM-модель (valuation.py) всё ещё от полного номинала.

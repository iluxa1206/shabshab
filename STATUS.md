...
- **Формат сообщения доведён** (см. память telegram-bot): шапка «спред · выпуск
  (срок) · объём», паспортная строка «цена · ISIN · рейтинг», стакан отбит
  пустыми строками, объём в стакане **в бумагах**, формула купона «КС + 1,45 %
  (12)» под книгой, подробности после неё.
- **Follow-up судьбы заявки** (`services/signal_followup.py`, `62083ef`): через
  15 минут бот отвечает РЕПЛАЕМ на сигнал — забрали / сняли / частично / стоит,
  плюс движение спреда. Исход считается по накопленному объёму до цены сигнала
  и ленте сделок Alor: осталось ≥80 % — «стоит», 20–80 % — «частично», меньше —
  ушла, и тогда «забрали», если сделками закрыта хотя бы треть исчезнувшего
  объёма. Только на «заявку», один ответ, вне торгов не проверяем, «стоит» —
  без звука. Нет исходного объёма → молчим, а не выдумываем исход.
...
0c), без z-спреда 27 из 1315; follow-up-очередь без зависших; расхождение

...
# перестала бы писаться. Держим последние сутки: пересборка из свежего кэша
# быстрая, а хвост не нужен никому.
echo ">>> чистка кэша сборки (старше 24ч)"
# shellcheck disable=SC2086
ssh $SSH_OPTS "$SERVER" "docker builder prune -f --filter until=24h 2>&1 | tail -1"

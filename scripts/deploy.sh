...
# Кэш сборки Docker растёт с КАЖДЫМ деплоем и молча съедает диск: 25.08 он
# занимал 9,2 ГБ при 5 ГБ свободного места — ещё несколько деплоев, и база
# перестала бы писаться.
#
# Ограничение по ВОЗРАСТУ (было `--filter until=24h`) этого не лечит: оно
# выбрасывает записи старше суток, а в день активных деплоев весь кэш свежий.
# Замер 25.08: при 9,2 ГБ к освобождению команда вернула `Total: 0B` — защита
# не срабатывала ровно в том сценарии, ради которого писалась.
#
# Потолок по ОБЪЁМУ работает всегда. 4 ГБ: хватает на горячий кэш нашего образа
# и соседних стеков (билдер на хосте общий — тут же собираются astra-prod,
# vacation-tracker, crm, tgbot), при этом steady-state диска остаётся ~2/3.
#
# Цифра НЕ жёсткая: записи, которыми подпёрты живые образы, не выбрасываются.
# Замер 25.08: потолок 4 ГБ осадил кэш с 9,2 до 5,87 ГБ и на повторном прогоне
# вернул `Total: 0B` — то есть сошёлся, оставив ~1,6 ГБ shared. Это нормально,
# не повод занижать порог: рост он всё равно ограничивает.
echo ">>> чистка кэша сборки (потолок 4 ГБ)"
# shellcheck disable=SC2086
ssh $SSH_OPTS "$SERVER" "docker builder prune -f --max-used-space 4GB 2>&1 | tail -1"

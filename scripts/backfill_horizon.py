...
* ЕСТЬ — пересчитываем честным as-of движком НА ИХ ЖЕ ЦЕНЕ (тот же путь, что
  ensure_honest_backfill): calc_date = дата строки, кривая/НКД/номинал того дня.
  Пишем y_idx (к выбранному горизонту), y_idx_alt (ко второму) и оба ключа.

  ГОРИЗОНТ ПРИ ЭТОМ ОДИН НА ВСЮ СЕРИЮ, а не «по правилу цены на ту дату»:
  honest_spread_series определяет hz_key по последней строке окна и штампует его
  на все точки. Так и надо — линия из точек с РАЗНЫМИ горизонтами несопоставима
  сама с собой, а восстановить второй горизонт всегда можно из y_idx_alt
  (этим и занимается _one_horizon на чтении).

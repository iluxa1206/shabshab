...
services/tape):
  • тиковый архив Alor — безадресные сделки в реальном времени: по флоатер-
    юниверсу любого размера, по остальному рынку от 1 млн ₽ (services/
    trades_stream);
  • сделки всего рынка из ISS — включая адресные режимы (РПС, РПС с ЦК,
    размещения, выкупы), которых в стакане нет вообще, но с задержкой публичного
    ISS ~15 минут.

Сюда в Alor мы не ходим: массовый онлайн-дрейн по 3000 бумагам на запрос
страницы упёрся бы в rate-limit брокера. Таблицы наливают фоновый стрим и демоны
(hourly_bars_worker и block_trades_worker), поэтому лента отстаёт от биржи не
больше, чем на их такт — а по безадресным сделкам не отстаёт вовсе.
...
async def tape(
    ...
    spread_min: Optional[float] = Query(None, description="spread сделки от, бп"),
    spread_max: Optional[float] = Query(None, description="spread сделки до, бп"),

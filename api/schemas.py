...
class CouponDayRow(BaseModel):
    ...
    ru_rate_src: Optional[str] = None   # fact | forward — источник ИМЕННО ставки RUONIA
    ru_index_src: Optional[str] = None  # fact | forward — источник уровня индекса RUONIA
...
class BondListItem(BaseModel):
    ...
    issue_volume: Optional[float] = None   # размещено штук × номинал, в валюте номинала (MOEX ISSUESIZEPLACED)
    next_coupon_date: Optional[date]
    last_price_pct: Optional[float]

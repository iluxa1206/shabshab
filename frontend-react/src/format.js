...
export const fmt = {
  ...
  // деньги в млн ₽ для колонок оборота: миллионы — рабочая единица тикета на
  // этом рынке, а полное число рублей (137 482 915) не влезает в колонку.
  // Меньше 0.1 млн печатаем «<0,1», иначе десятки строк схлопываются в «0,0»
  mln: (v) => (v == null ? null
    : v === 0 ? "0"
    : Math.abs(v) < 1e5 ? "<0,1"
    : (v / 1e6).toLocaleString("ru-RU", { minimumFractionDigits: 1, maximumFractionDigits: 1 })),
};

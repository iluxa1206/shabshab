...
function Dashboard() {
  ...
  // последний известный spread на бумагу: держит строку в окне фильтра, пока
  ...
  useEffect(() => {
    ...
    setBonds((prev) => {
      ...
      const next = prev.map((b) => {
        ...
        // spread застревал на значении, приехавшем при загрузке страницы.
        ...
        if (q.last != null) {
          // spread посчитан к ПРЕЖНЕЙ цене сделки: пока движок не прислал
  ...
  // ВИДИМЫЙ СРЕЗ — движку. Он обслуживает весь рынок, но ждут именно эти
  // строки: по ним он греет контекст расчёта, стороны стакана и сетку цен в
  // первую очередь, а хвост рынка догревает в свободное время. Шлём верх
  // отфильтрованного списка — то, что человек видит без прокрутки и сразу под
  // ней; дальше сервер сам режет по своему потолку.
  useEffect(() => {
    const t = setTimeout(
      () => wsRef.current?.setVisible(tableRows.slice(0, 150).map((b) => b.isin)), 500);
    return () => clearTimeout(t);
  }, [tableRows]);
    ...
    <div id="app" className={theme === "light" ? "" : "theme-" + theme}>
      ...
      <Routes>
        ...
        {/* Вкладка «Эмитенты» снята: агрегаты повторяли фильтр по эмитенту в
            ...
            spread по эмитентам). Старый путь ведёт в Монитор — на вкладку могли
            остаться закладки. */}

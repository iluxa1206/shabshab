...
export default function SpreadHistory({ isin, kind, secid, board, days: daysProp, from, syncDate, onHoverDate }) {
  ...
  const label = isFixed ? "G-спред" : "spread";
  ...
  return (
    <div className="sh-box">
      ...
      <div className="sh-note">
        ...
        {q.data?.skipped_no_trades > 0 && (
          <> Дни без сделок пропущены ({q.data.skipped_no_trades}) — спред считаем только там, где была торговля.</>
        )}
      </div>
    </div>
  );

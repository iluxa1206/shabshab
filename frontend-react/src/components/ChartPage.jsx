...
export default function ChartPage() {
  ...
  // ТАЙМФРЕЙМ. По умолчанию ЧАСОВОЙ — на окне месяца он и есть рабочий масштаб,
  // дневка там склеивает торговый день в одну свечу и прячет внутридневное
  // движение. Часовой доступен только там, где под ним есть данные (окно ≤ 45
  // дней: глубина часовых свечей ISS), поэтому на широких окнах дефолт сам
  // сваливается в дневки — как и неизвестный или недоступный tf в URL.
  ...
  const tf = TFS.some(([k]) => k === tfRaw) && tfOk(tfRaw) ? tfRaw
    : (tfOk("1h") ? "1h" : "1d");

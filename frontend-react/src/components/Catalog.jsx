...
import { filterBonds, normalize } from "../search.js";
...
const COLS = [
  ...
  ["var_type", "Тип ставки"],
  ["coupon_text", "Формула"],
  // Лесенка маржи по номерам купонов («7-20=400»): купоны вне диапазонов
  // считаются НЕ плавающими — скаляр margin_bps, бэктест их не судит.
  ["margin_schedule", "Лесенка маржи"],
  // ЛИНКЕР: база индексации НОМИНАЛА ('RUONIA' | пусто). У такой бумаги ставка
  // купона фиксирована (её держит margin_bps), а по индексу растёт номинал.
  // Проставляется автодетектом, поле — ручной оверрайд.
...
export default function Catalog({ user }) {
  ...
  const rows = useMemo(() => {
    ...
    items = filterBonds(items, query, (r) => ({
      name: normalize(r.short_name || ""), isin: normalize(r.isin || ""),
    }));

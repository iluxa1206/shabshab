...
/**
 * ПЛАШКА события: что именно случилось. Две принципиально разные новости, и
 * ярлык обязан их разводить с первого взгляда.
 *
 * Заявка (событие стакана) — всегда «заявка», а при повторе в скобках стоит
 * ПАРАМЕТР, который сдвинулся: «заявка (спред)». Раньше повтор подписывался
 * одним словом «спред»/«объём», и строка читалась как новость про рынок, хотя
 * это та же заявка, пересобранная под тем же фильтром.
 *
 * Сделка (крупный принт) — «сделка» + сторона агрессора буквой: B купил,
 * S продал. Прошлый ярлык «блок» называл механику фильтра, а не событие.
 * У адресной (РПС) агрессора нет — остаётся голое «сделка».
 */
export function eventTag(e) {
  if (!e) return "";
  if (e.reason === "block") {
    // адресная идёт без буквы ДАЖЕ если сторона в строке есть: у РПС стороны
    // договорились заранее, агрессора нет, и буква утверждала бы неправду
    if (e.negotiated) return "сделка";
    if (e.side === "buy") return "сделка B";
    if (e.side === "sell") return "сделка S";
    return "сделка";
  }
  const param = { spread: "спред", money: "объём", price: "цена" }[e.reason];
  return param ? `заявка (${param})` : "заявка";
}

...
def test_side_price_falls_back_to_board_snapshot():
    """Сторона берётся из пуша, а чего в нём нет — из биржевого снапшота.

    Пуш Alor приходит и без стороны (тик сделки), а прежняя логика включала
    фолбэк, только если пуша не было ВОВСЕ. В итоге спред стороны не считался,
    а цену в строку тут же клал поллер из снапшота: строка «цена есть, спреда
    нет» висела до следующего движения книги — на проде 02.09 таких было 75."""
    snap = {"bid": 99.79, "ask": 100.23}
    # тик сделки: цены сделки есть, сторон в пуше нет
    assert us._sides_from({"last_price": 100.0}, snap) == {"bid": 99.79, "ask": 100.23}
    # пуш со своей стороной — она и побеждает, снапшот только добирает вторую
    assert us._sides_from({"bid": 99.5}, snap) == {"bid": 99.5, "ask": 100.23}
    # пуша не было вовсе — весь верх книги из снапшота (прежнее поведение)
    assert us._sides_from(None, snap) == {"bid": 99.79, "ask": 100.23}
    # ни там, ни там — стороны нет, и это честный прочерк
    assert us._sides_from({}, {}) == {"bid": None, "ask": None}

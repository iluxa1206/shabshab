...
### Данные / инфра
- [ ] Рейтинг-драйн: corpbonds часто 404 → `r is None` → не кэшируется → эти бумаги ретраятся каждый проход поллера (остаточный долбёж, не блокирует запросы). Фикс: negative-кэш 404 с коротким TTL, чтобы не долбить постоянно. (Батч /api/fixed + дедуп по реестру уже сделаны — коммит perf(ratings).)

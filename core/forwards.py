...
class DiscountCurve:
    ...
    def daily_forward(self, d: date) -> float:
        """Уровень базы на дату d — СТУПЕНЬ между тенорами: форвард ЦЕЛОГО
        сегмента кривой, в который попадает d. Внутри сегмента константен,
        скачет на узле (как реальная КС между заседаниями) → кусочно-флэт путь
        для ПО-ДНЕВНОЙ проекции индекса (спека фиксинга, дневная раскладка).

        Именно сегментное окно, НЕ forward(d, d+1): на однодневном окне
        конвенция ставки расходится с уровнем индекса (simple за 1 день ≈
        непрерывная ставка: при квартальном номинале 15% выходило 14.72% —
        инвариант C2 valuation↔zspread ловил это расхождением потоков started-
        купонов). Окно сегмента сохраняет конвенцию узловых котировок.
        За последним узлом — форвард последнего сегмента (flat-экстраполяция)."""
        lo = max(d, self.calc_date)
        for i in range(1, len(self.nodes)):
            if self.nodes[i][0] > lo:
                return self.forward(self.nodes[i - 1][0], self.nodes[i][0])
        if len(self.nodes) >= 2:
            return self.forward(self.nodes[-2][0], self.nodes[-1][0])
        return self.forward(lo, lo + timedelta(days=1))

    def rate_bounds(self) -> List[date]:
        """Даты, на которых daily_forward() меняет ступень (первая точка каждой
        новой ступени, calc_date исключён).

        Потребители, шагающие по пути индекса (роллирование RUONIA), кэшируют
        уровень между границами вместо вызова daily_forward() на каждый день.
        База: ступень = сегмент между узлами. Кривые с БОЛЕЕ ЧАСТОЙ ступенью,
        чем их узлы (гибрид as-of: дневной факт индекса внутри одного узла),
        ОБЯЗАНЫ переопределить — иначе потребитель заморозит первый уровень на
        весь горизонт.
        """
...
class BootstrappedForwardCurve(DiscountCurve):
    ...
    def _equivalent_rate(self, factor: float, days: int) -> float:
        ...
        # KEYRATE simple ACT/365 — ТОЧНАЯ ставка одиночного купонного периода
        # (1+f·days/365)=factor: потребитель множит на days/365 и воспроизводит
        # factor. «Раздутой» она выглядит лишь если читать как компаундированный
        # annualized (10Y-пролёт ~35%). Годовой купон (366д) и короче — легитимны.
        # Порог 370д ловит только СВЕРХгодовые пролёты (2Y+ одним вызовом) —
        # это уже мисьюз (проекция купона зовёт forward() смежными ≤период-сегментами,
        # телескопирование). ВНИМАНИЕ: 366д (смежные 1Y→2Y) под порогом НЕ алармят —
        # это by design (годовой купон валиден), НЕ признак что simple там «раздут».
        if days > 370:
            logger.warning(
...
class SheetForwardCurve(DiscountCurve):
    ...
    КОНВЕНЦИЯ ФИКС-НОГИ — ГОДОВАЯ, И ЭТО ОСОЗНАННЫЙ ВЫБОР (решение 2026-08-26).
    Проверка репрайсом котировок: пересчёт `4·((1+par)^¼−1)` точно воспроизводит
    свопы при ГОДОВОЙ фикс-ноге (1Y 14.044 против котировки 14.040, 10Y 14.944
    против 15.100 — расхождение только на телескопировании), а при квартальной
    ноге репрайс требует `avg = par` тождественно.

    ЗДЕСЬ РАСХОЖДЕНИЕ С CurveBootstrapper._bootstrap_par_curve, где для KEYRATE
    стоит months=3 (квартальная нога, конвенция СПФИ МБ). Это НЕ ошибка одной из
    сторон: бутстрап решает другую задачу (безарбитражные DF из котировок), а
    здесь воспроизводится методика трейдерского листа, утверждённая как источник
    будущих ставок. НЕ «чините» sheet под bootstrap и наоборот — числа разойдутся
    на 70-90 bps, а Y-IDX всех KEYRATE-бумаг уедет на +73…+91 bps.

    Та же формула ПРОДУБЛИРОВАНА в api/routes/curves.py::_avg_pct (вкладка
    КРИВЫЕ). Правки — только синхронно, иначе график и прайсинг разойдутся.
    ...
    def __init__(self, calc_date: date, quotes: List, base_type: str):
        ...
        def _avg(par: float, dd: int) -> float:
            ...
            # ГОДОВАЯ фикс-нога — см. докстринг класса. Не менять в отрыве от
            # api/routes/curves.py::_avg_pct.

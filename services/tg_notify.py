...
_ELLIPSIS = "..."   # порог штук убрал уровни между лестницей и спредом
...
def _book_pre(m: dict, side: Optional[str]) -> str:
    ...
    #
    # МНОГОТОЧИЕ — ТОЛЬКО У КОТИРОВКИ, между показанной лестницей и спредом.
    # Там оно отвечает на вопрос, который читатель задаёт первым: «99,70 —
    # правда лучший оффер?» Если порог убрал заявки ближе к спреду, то нет, и
    # цена, до которой надо дойти, другая.
    #
    # Пропуски В СЕРЕДИНЕ лестницы не помечаются, хотя снимок их считает:
    # глубина и так читается как «примерно столько-то по таким-то ценам», от
    # выпавшей середины этот ответ не меняется, а метки в каждой второй строке
    # рвут колонку ради ничего. Глубина ЗА лестницей не помечается тем более —
    # она обрезана всегда.
    #
    # Сторона печати: у офферов ближайший к спреду уровень идёт ПОСЛЕДНИМ
    # (лестница печатается от худшей цены), у бидов — ПЕРВЫМ.
    lines = [row(l) for l in asks]
    if (asks and asks[-1].get("gap")) or book.get("asks_all_hidden"):
        lines.append(_ELLIPSIS)
    # 14, а не по ширине строк: на телефоне сворачиваемая цитата уже колонок,
    # и разделитель во всю ширину переносился на вторую строку — рвал блок.
    lines.append("─" * 14)          # выше разделителя оффера, ниже биды
    if (bids and bids[0].get("gap")) or book.get("bids_all_hidden"):
        lines.append(_ELLIPSIS)

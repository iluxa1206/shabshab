...
def _money_of(m: dict, kind: str) -> Optional[float]:
    ...
    У ЗАЯВКИ это деньги ПО ЦЕНЕ СИГНАЛА — весь объём уровня (или уровней, если
    набор занял несколько), `level_money_rub`. Два соседних кандидата не
    годятся: `money_rub` в режиме порога равен примерно самому порогу («>1 млн»
    → «1м ₽»), то есть повторяет настройку; `money_ok_rub` без границ спреда —
    это ВСЯ сторона стакана (Газпн3P13R 24.08: 20,7м в шапке против 3,8м на
    уровне 99,86, по которому и сработало). Фолбэки — на случай событий из
    ленты, где нового поля нет.

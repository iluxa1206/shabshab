...
_ELLIPSIS = "..."   # пропуск уровней в лестнице (порог штук)
...
def _book_pre(m: dict, side: Optional[str]) -> str:
    ...
    #
    # МНОГОТОЧИЕ — там, где порог штук выкинул уровни. Без него лестница с
    # вырезанной серединой читается как сплошная, и «лучший оффер 99,70» —
    # неправда, если между ним и спредом стояла мелочь. Пропуск всегда рисуется
    # СО СТОРОНЫ КОТИРОВКИ: у офферов под строкой, у бидов над ней, — потому
    # что gap уровня считается от лучшей цены, а офферы печатаются наоборот.
    # Глубина книги за пределами лестницы многоточием НЕ помечается: она
    # обрезана всегда, и метка стояла бы в каждом сообщении, ничего не говоря.
    lines = []
    for l in asks:
        lines.append(row(l))
        if l.get("gap"):
            lines.append(_ELLIPSIS)
    if book.get("asks_all_hidden"):
        lines.append(_ELLIPSIS)     # сторону выкосило целиком — прикрепить не к чему

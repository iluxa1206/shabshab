...
def calculate_valuation_metrics(
    ...
    # НКД ПРИШЁЛ ИЗ СТЕЙЛ-КЭША, А НЕ С БИРЖИ. accrued_override — это живой
    # снапшот; когда его нет (ISS недоступен), остаётся bond.accrued_rub из
    # isins_cache, а он может быть какой угодно давности. Ниже такой НКД
    # сверяется с графиком — молча доверять ему нельзя (см. _CACHE_ACC_TOL).
    accrued_from_cache = accrued_override is None and bond.accrued_rub is not None
    ...
    from core.valuation import (settle_date as _sd_ex, accrue_to_settle as _ats,
                                accrued_at as _acc_at, period_at as _period_at)
    ...
    # НКД ИЗ СТЕЙЛ-КЭША СВЕРЯЕМ С ГРАФИКОМ. Пока ISS жив, живой снапшот
    # перекрывает isins_cache и его возраст не виден. 10.09.2026 ISS лёг на
    # несколько часов — и расчёт молча поехал на НКД полуторамесячной давности:
    # Газпн3P10R получил 6,51 ₽ (кэш от 28.07) вместо 10,90 ₽ по графику, и
    # Y-IDX выдал 228 bps против 112 верных при той же цене. Никакого
    # предупреждения при этом не было: число «не None» — значит источник как бы
    # есть. Ошибка в 4 ₽ на бумаге с mod_duration 0,35 стоит ~126 bps.
    #
    # График — не догадка: купон текущего периода уже опубликован, и НКД по
    # нему точен. Расходится с кэшем сильнее допуска — верим графику.
    if accrued_from_cache and periods and accrued is not None:
        _ref_acc = _acc_at(periods, calc_date)
        _per = _period_at(periods, calc_date)
        # ДОПУСК МЕРЯЕМ В ДНЯХ НАЧИСЛЕНИЯ, не в рублях: биржевой НКД дан на свою
        # дату поставки, а график — на calc_date, и расхождение в 1-3 дня (T+1,
        # выходные, праздники) законно. Порог в рублях пришлось бы подбирать под
        # каждую ставку и длину периода; в днях он один на все выпуски.
        if _ref_acc is not None and _per and _per[2]:
            _daily = _per[2] / ((_per[1] - _per[0]).days or 1)
            _gap_days = abs(accrued - _ref_acc) / _daily if _daily else 0.0
            if _gap_days > _CACHE_ACC_MAX_DAYS:
                warnings.append(
                    f"НКД из кэша ({accrued:.2f} ₽) разошёлся с графиком "
                    f"({_ref_acc:.2f} ₽ на {calc_date.isoformat()}, "
                    f"{_gap_days:.0f} дн начисления) — взят график; "
                    "живой НКД биржи недоступен")
                accrued, accrued_date = _ref_acc, calc_date
                accrued_basis = "calc"
...
# Допуск расхождения НКД из isins_cache с графиком, В ДНЯХ НАЧИСЛЕНИЯ.
# Законный разрыв — дата поставки против calc_date: T+1, плюс выходные и
# праздники, максимум ~4 дня. Пять — с запасом на длинные выходные; всё, что
# больше, означает протухший кэш (10.09.2026: 9,5 дня на Газпн3P10R).
_CACHE_ACC_MAX_DAYS = 5.0

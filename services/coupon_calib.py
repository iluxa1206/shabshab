...
# Два независимых признака вместо одной фразы: «есть номера купонов» и «где-то в
# тексте упомянут индекс». Рядом их держать нельзя — в проспекте индекс живёт за
# точкой и под своей буквой («17-20 купоны: Кi = R + 2,5%, где … R - Ключевая
# ставка …»), а буква у каждого эмитента своя (R, Ij, Gj, Cr, Cк(i)).
_COUPON_NO_RE = re.compile(r"\d+\s*(?:[-–—]\s*\d+\s*)?купон", re.IGNORECASE)
_INDEX_MENTION_RE = re.compile(
    r"КЛЮЧЕВ|\bКС\b|RUONIA|РУОНИА|СТАВК[АИ]\s+ЦБ|СТАВК[АИ]\s+БАНКА\s+РОССИИ"
    r"|ИПЦ|ИНФЛЯЦ|РЕФИНАНСИР", re.IGNORECASE)


def mentions_floating_tranche(text: Optional[str]) -> bool:
    """В тексте формулы есть ДИАПАЗОН КУПОНОВ с плавающей ставкой?

    Смешанная бумага («1-16 купоны — 9.5%, 17-20 купоны: R + 2,5%») ведёт себя
    как фикс по всем реализованным купонам, поэтому looks_fixed_coupons на ней
    ложно срабатывает — а бумага уже плавает (Башнефть БО-10: купоны 17-20 идут
    как раз сейчас, а вердикт FIXED уводил её в ФИКСЫ с ложным YTM).

    Признак нужен именно как ВЕТО на автоматический FIXED, а не как детект
    флоатера: базу и маржу дальше определяет обычная цепочка. Поэтому правило
    сознательно широкое — «в тексте есть номера купонов И упомянут индекс».
    Ложное вето оставляет бумагу в ревью, ложный FIXED уводит её в ФИКСЫ с
    неверным YTM; цена ошибок разная.

    Чистый фикс так не ловится: у него в формуле индекса нет вовсе
    («1-20 купоны — 12.5% годовых»)."""
    if not text:
        return False
    t = str(text)
    return bool(_COUPON_NO_RE.search(t) and _INDEX_MENTION_RE.search(t))

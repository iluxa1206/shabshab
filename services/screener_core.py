...
def book_snapshot(depth_side: Optional[dict], row: dict, face: float,
    ...
    def side_levels(key: str) -> list:
        ...
        витрине: «лучший оффер 99,70» — неправда, если между ним и спредом
        стояли мелкие заявки. Телеграм рисует многоточием ТОЛЬКО пропуск у
        котировки (gap ближайшего к спреду уровня), середину лестницы считаем,
        но не помечаем — см. tg_notify._book_pre."""
        out, skipped = [], 0

...
def markets_to_sweep() -> tuple:
    """Какие ленты ISS читать. СЕЙЧАС — ВСЕГДА ОБЕ, и вот почему.

    Казалось бы, при живом стриме Alor безадресная лента (bonds) лишняя: те же
    сделки приходят пушем мгновенно, а ISS привозит их с задержкой 15 минут,
    склейка по TRADENO дублей не делает. 11.09.2026 попробовали читать только
    ndm, добирая bonds раз в 15 минут ради поля yld — и получили «ЛЕНТА
    ОТСТАЁТ: живьём поймано 27 из 247 крупных сделок за час».

    Разгадка в том, ЧТО ИМЕННО пишет стрим. В block_trade (лента крупных сделок
    и очередь звонка) из стрима попадают только сделки ВЫШЕ alert_floor —
    порога активных получателей. Всё, что между 1 млн и этим порогом, живёт
    исключительно за счёт ленты ISS, и с её тактом раз в 15 минут такие сделки
    доезжали до ленты за полчаса вместо пятнадцати минут.

    Цена отказа от bonds — 408 запросов в сутки из 2 728. Полнота ленты этого
    не стоит. Функцию оставляем: она документирует проверенную гипотезу, чтобы
    её не предлагали заново.
    """
    return MARKETS

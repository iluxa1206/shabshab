...
def seed_begin(market_cache: dict, calc_date) -> None:
    """Прогрев НАЧИНАЕТ засев — версию объявляем заранее.

    Именно заранее, а не по окончании: движок просыпается через 40 с после
    старта, а прогрев с пустым кэшем расписаний качает 611 bondization и длится
    дольше. Первый такт приходится на СЕРЕДИНУ засева, и отметка, поставленная
...
def seed_count() -> int:
    """Сколько контекстов реально осталось у движка — число для лога. Раньше
    печаталось len(_eval_ctx) сразу после прогрева и жило до первого такта."""
...
def _check_version(version: tuple, ctx: Optional[dict] = None) -> None:
    ...
    if version != _memo_version:
        ...
        # ДЕНЬ ЗАСЕВА ТОЖЕ СЧИТАЕТСЯ. На старте и после переката _memo_version
        # пуста, и «тот же день» определить было нечем — любая пересборка кривых
        # во время прогрева уходила в тотальный снос вместо перепривязки. А
        # контекст кривой НЕ ПРИНАДЛЕЖИТ: от неё в нём одна ссылка, всё
        # остальное (ref_obj, купоны, амортизации, оферты, НКД) на новой кривой
        # ровно то же. Репетиция переката 02.09: посчитано 611, у движка осталось
        # 315 — половину снесло на середине прохода.
        if not same_day and _seeded_version and _seeded_version[0] == version[0]:
            same_day = True

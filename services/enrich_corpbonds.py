...
def _parse_formula(f: str) -> dict:
    ...
    # инверсный: «X% − КС» (индекс вычитается из числа) — линейной моделью не
    # считаем. Индекс в проспектах зовут по-разному, вплоть до одной буквы:
    # АЛЬФАБ1Р11 «max (25.90% - R; 12.90%)» проходил мимо шаблона на «КС» и
    # считался как КС+25,9 % — 40 % годовых вместо реальных 12,9 %.
    inverse = bool(re.search(r"[-−–]\s*(КС|KC|CBR|RUONIA)", up)
                   or re.search(r"[\d.,]+\s*%?\s*[-−–]\s*R\b(?!UONIA)", up))

...
def _on_trade(isin: str, data: dict) -> None:
    ...
    #
    # ФИКСЫ порогу ПОДЧИНЯЮТСЯ, хотя живой счёт получают целиком: их тик всё
    # равно доедет в архив плановым добором (bars.refresh_universe →
    # trades_archive.drain по kinds=floater,fixed — на проде это ~350 тыс. строк
    # в день, и было так до появления фиксов в стриме). Дублировать тот же объём
    # ещё и потоком незачем: в ленте по фиксам крупняк живой, остальное приезжает
    # добором. Нужна лента фиксов тик-в-тик — снимать порог здесь, а не считать,
    # что архив от этого вырастет.

...
# Сколько минут тишины на живых сокетах ещё считаем нормой. Порог щедрый:
# в неликвидной бумаге сделок может не быть часами, но по ВСЕМУ рынку
# облигаций пять минут без единого принта в торговое время означают, что
# стрим встал, а не что рынок замер.
_COVER_QUIET_MIN = 5.0
# Доля живых шардов, ниже которой считаем покрытие дырявым.
_COVER_MIN_SHARE = 0.8


def covers_market() -> bool:
    """Покрывает ли живой стрим безадресные сделки всего рынка.

    Нужно, чтобы не тянуть из ISS то, что и так приходит пушем: сквозная лента
    market=bonds дублирует ровно эти сделки, только с задержкой 15 минут
    (склейка по TRADENO — дублей нет, но и новой информации тоже). Адресные
    (ndm) стрим не покрывает никогда: подписки на них у брокера нет.

    Признак «стрим жив» — большинство шардов подняты И по рынку недавно был
    тик. Одного числа шардов мало: сокет может висеть открытым и молчать
    (прод 28.08.2026 — «сторож видит живых», а данных нет).
    """
    if not _streamed or not _shards:
        return False
    up = sum(1 for s in _shards.values() if s.get("up"))
    if up < _COVER_MIN_SHARE * len(_shards):
        return False
    # отметка берётся из ШАРДОВ (epoch), а не из _stats["last_ts"] — там строка
    # для показа человеку, арифметике она не годится
    last = max((s.get("last") or 0) for s in _shards.values())
    if not last:
        return False
    return (time.time() - last) / 60.0 <= _COVER_QUIET_MIN
...
def _on_trade(isin: str, data: dict) -> None:
    ...
    #
    # ФИКСЫ порогу НЕ подчиняются — как и флоатер-юниверс: у них своя витрина и
    # своя лента, а сделка должна быть видна сразу, а не через час. Объём базы от
    # этого не растёт: те же тики всё равно доезжают плановым добором
